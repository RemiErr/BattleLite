...
impl GGRSSession {
    ...
    fn advance_local(&mut self, inputs: Vec<u8>) -> PyResult<()> {
        ...
        for i in 0..self.current_state.players.len() {
            ...
            ggrs_style_inputs.push((val, InputStatus::Confirmed));
        }
        self.apply_logic(&ggrs_style_inputs);
        Ok(())
    }
    ...
    fn set_player(&mut self, player_id: usize, player: Player) -> PyResult<()> {
        if let Some(p) = self.current_state.players.get_mut(player_id) { *p = player; Ok(()) } else { Err(PyIndexError::new_err("Out of range")) }
    }
}
